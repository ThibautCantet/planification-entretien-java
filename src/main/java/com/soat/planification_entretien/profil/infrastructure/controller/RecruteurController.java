...
import com.soat.planification_entretien.profil.use_case.CreerRecruteur;
import com.soat.planification_entretien.profil.use_case.ListerRecruteursExperimentes;
...
public class RecruteurController {
    ...
    public ResponseEntity<List<RecruteurDetailDto>> lister() {
        List<RecruteurDetailDto> recruteurs = listerRecruteursExperimentes.execute().stream()
                .map(e -> new RecruteurDetailDto(e.getId(), e.getLanguage(), e.getExperienceInYears(), e.getEmail(), e.isDisponible()))
                .toList();
